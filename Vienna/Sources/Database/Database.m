...
- (BOOL)setupInitialDatabase {
	...
	[self.databaseQueue inDatabase:^(FMDatabase *db) {
		...
		[db executeUpdate:@"insert into info (first_folder, folder_sort) values (0, ?)", @(MAFolderSortManual)];
		[[Preferences standardPreferences] setFoldersTreeSortMethod:MAFolderSortManual];
	}];
...
-(NSInteger)addFolder:(NSInteger)parentId afterChild:(NSInteger)predId folderName:(NSString *)name type:(NSInteger)type canAppendIndex:(BOOL)canAppendIndex
{
	...
	BOOL manualSort = [Preferences standardPreferences].foldersTreeSortMethod == MAFolderSortManual;
	if (manualSort)
	{
		if (predecessorId > 0)
		{
